...
\begin{quote}
    ...
    As such, alternative non-linear approaches have been proposed for modeling univariate densities, e.g., transformations that map the densities to an unconstrained space before doing FPCA \textbf{(CITE PETERSEN)}.
    However, we have chosen to apply standard FPCA, acknowledging its limitations, because:
    \begin{enumerate}
        \item Conventional FPCA is tractable, it is straightforward to describe to a general data-science audience. We can interpret the individual FPCs as a shift of the mean density \textbf{(CITE KNEIP)}. Furthermore, standard computational FPCA methods can be employed to deal with the high-dimensionality of the two-dimensional surfaces.
        \item Although specialized geometrically-aware density approaches can be can give better performance, standard FPCA has been shown to provide a reasonable approximation in many cases  \textbf{(CITE PETERSEN) (CITE DAI and Muller)}.
        \item If FPCA approximations of individual densities take negative values, they can be projected back into the density space, by retaining only the positive part and re-normalizing.  \textbf{(CITE PETERSEN) (CITE HALL)}
    \end{enumerate}
    }
