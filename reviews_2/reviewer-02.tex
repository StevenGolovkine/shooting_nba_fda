...
\reply{
  ...
  We have also clarified that the choice of the method is not intrinsic to the proposed framework. The representation provides a general low-dimensional summary of each player's shooting pattern and different clustering algorithms could be used depending on the goal of the analysis. For example, hierarchical clustering is useful when we want to study nested similarities between players; mixture models is useful when a probabilistic assigmnent is desired and density-based method is useful to identify outliers.
